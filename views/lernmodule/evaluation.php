<form class="default studip_form">
    ...
    foreach ($attempts as $attempt) {
        if ($attempt['successful']) {
            $successful['variance'] += (pow(
                ($attempt['chdate'] - $attempt['mkdate']) - $successful['average'],
                2
            ) / $successful['count']);
        } else {
            $unsuccessful['variance'] += (pow(
                ($attempt['chdate'] - $attempt['mkdate']) - $unsuccessful['average'],
                2
            ) / $unsuccessful['count']);
        }
    }
    ...
    $max = max(
        round(
            max(
                $successful['average'] + $successful['standarddeviation'],
                $unsuccessful['average'] + $unsuccessful['standarddeviation']
            ) / 10
        ) * 10,
        60
    );
    ...
    ?>
    <?php if ($successful['count'] + $unsuccessful['count'] > 0) : ?>
      <fieldset>
        ...
        "Zeit der Durchläufe in Sekunden bzw. Minuten. Blau sind die erfolgreichen Durchläufe, orange die nicht erfolgreichen."
          ...
          <?php if (is_a($module, "CustomLernmodul")) : ?>
              <?php $template = $module->getEvaluationTemplate(Context::get()->id) ?>
              <?php if ($template) : ?>
                  <?= $template->render() ?>
              <?php endif ?>
          <?php endif ?>
      ...
      <table class="default" id="resulttable">
        ...
        <tr>
          ...
          <th><?= dgettext("lernmoduleplugin", "Datum") ?></th>
            <?php foreach ($resultrows as $rowname) : ?>
        ...
        <?php foreach ($data as $line) : ?>
          <tr>
            <td>
              <div style="width: 50px; height: 50px; background-image: url('<?= Avatar::getAvatar(
        $line['studip_user_id']
    )->getURL(Avatar::MEDIUM) ?>'); background-size: 100% auto; background-position: center center;"></div>
            </td>
            <td><?= htmlReady(
        $line['studip_user_id'] ? get_fullname($line['studip_user_id']) : dgettext(
                    "lernmoduleplugin",
                    "Anonym"
                )
    ) ?></td>
            <td data-timestamp="<?= htmlReady($line['studip_duration']) ?>"><?php
      ...
      <script>
        jQuery(function() {
          jQuery('#resulttable').tablesorter({
            ...
            cssAsc: 'sortasc',
            cssDesc: 'sortdesc'<?php if (count($resultrows > 2)) : ?>,
            sortList: [[3, 0]]
              <?php endif ?>
          });
        });
